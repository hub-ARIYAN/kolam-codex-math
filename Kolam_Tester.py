...
def analyze_real_kolam(image_path):
    ...
    try:
        ...
        create_analysis_visualization(img, result, base_name)
...
def create_analysis_visualization(original_img, result, base_name):
    ...
    try:
        ...
        # Save visualization
        viz_output = f"{base_name}_analysis_visualization.png"
...
def main():
    ...
    # Analyze the kolam
    result = analyze_real_kolam(image_path)
    
    if result and 'error' not in result:
        print("\n✅ ANALYSIS COMPLETED SUCCESSFULLY!")
        # Show key insights
        dots_count = len(result.get('dots', []))
        lattice = result.get('lattice')
        cycles_count = len(result.get('cycles', []))
        
        print(f"\n🔍 KEY INSIGHTS:")
        print(f"   • Found {dots_count} dots in the pattern")
        if lattice:
            print(f"   • Detected {lattice['type']} lattice structure")
        print(f"   • Identified {cycles_count} closed loops/cycles")
        
        symmetry = result.get('symmetry', {})
        if symmetry.get('rotational_order', 1) > 1:
            print(f"   • Has {symmetry['rotational_order']}-fold rotational symmetry")
        if symmetry.get('mirror_axes', 0) > 0:
            print(f"   • Has {symmetry['mirror_axes']} mirror symmetries")
    
    else:
        print("\n❌ Analysis failed or encountered errors.")
